...
//		desktop.getSplashScreen().appendMessage("\n\n" + d);
		...
		try {
			...
			// TitoStatus status = titoSession.openSession("tito", "tito");
	...
	private synchronized void login()
	{
		...
		// TraderLoginDialog loginDialog = getLoginDialog(desktop);
		GridLoginFrame loginFrame = getLoginFrame();
		...
		desktopTitle = "Pado Desktop (" + getPaodUri(envName, appId, username, locators) + ")";
	...
	// protected AccountManagerDialog getAccountManagerDialog()
	// {
	// if (accountManagerDialog == null) {
	// Frame frame = desktop.findAncestorFrame(desktop);
	// accountManagerDialog = new AccountManagerDialog(frame,
	// "Account Manager Dialog", true);
	// if (frame == null) {
	// accountManagerDialog.setLocation(100, 100);
	// } else {
	// int x = frame.getLocationOnScreen().x + frame.getSize().width / 2
	// - accountManagerDialog.getSize().width / 2;
	// int y = frame.getLocationOnScreen().y + frame.getSize().height / 2
	// - accountManagerDialog.getSize().height / 2;
	// accountManagerDialog.setLocation(x, y);
	// }
	// }
	...
	public class GridDesktopInitCommandProvider implements ICommandProvider
	{
		...
		// private VolatilityPanel volatilityPanel = null;
		// private ProbabilityPanel probabilityWindow = null;
		// private ValueAtRiskPanel varPanel = null;
		// private ValueToGainPanel vtgPanel = null;
		// private PredictionReturnPanel highsLowsPanel = null;
		// private PointPredictionPanel pointPredictionPanel = null;
		// private AccountSummaryPanel accountSummary = null;
		// private TransactionHistoryPanel transactionHistory = null;
		// private PnLStatementPanel pnlStatement = null;
		...
		private CommandDescriptor commandDescriptors[] = {
				// new CommandDescriptor("openSingleStockTrading",
				// "Opens the SingleStockTrading window."),
				// new CommandDescriptor("openBasketTrade",
				// "Opens the BasketTrade window."),
				// new CommandDescriptor("openArbitraage",
				// "Opens the Arbitrage window."),
				// new CommandDescriptor("openWatchList",
				// "Opens the WatchList window."),
				// new CommandDescriptor("openPortfolio",
				// "Opens the Portfolio window."),
				// new CommandDescriptor("openMarketIndexChart",
				// "Opens the Index Chart window."),
				// new CommandDescriptor("openTradeHistory",
				// "Opens the Trade History window.")
		};
		...
		private void openWorkspace(Component comp, int x, int y, int width, int height)
		{
			...
			if (internalFrame == null) {
				...
				if (detachedFrame == null) {
					...
					// internalFrame.addInternalFrameListener(new
					// javax.swing.event.InternalFrameAdapter() {
					// public void internalFrameClosing(InternalFrameEvent e)
					// {
					// workspace_internalFrameClosing(e);
					// }
					// });
