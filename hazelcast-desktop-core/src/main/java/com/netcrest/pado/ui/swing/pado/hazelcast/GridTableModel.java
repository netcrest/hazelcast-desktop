...
public class GridTableModel extends ObjectTableModel
{
			...
			throws IllegalArgumentException, IllegalAccessException, InvocationTargetException
	{
		...
		if (dataObject instanceof KeyMap) {
			...
			resetMapList(firstRowNum, objList);
		} else if (isPrimitiveType(objList)) {
			resetPrimitiveList(firstRowNum, objList);
		} else {
			resetObjectList(firstRowNum, objList);
		}
	}
	
	private void resetPrimitiveList(int firstRowNum, List objList) {
		removeAll();
		if (objList == null || objList.size() == 0) {
			return;
		}
		...
		String columnNames[];
		if (isRowNumberAdded()) {
			columnNames = new String[] { "Row", className };
		} else {
			columnNames = new String[] { className };
		}
		...
		for (int i = 0; i < objList.size(); i++) {
			rowData = new Object[1 + firstColumn + 1];
			if (isRowNumberAdded()) {
				rowData[0] = firstRowNum + i;
			}
			Object obj = objList.get(i);
			rowData[firstColumn] = obj;
			// user data
			rowData[firstColumn + 1] = obj;
			addRow(rowData);
		}
	}
	
	private boolean isPrimitiveType(List objList) {
		if (objList == null || objList.size() == 0) {
			return true;
		}
		Object dataObject = objList.get(0);
